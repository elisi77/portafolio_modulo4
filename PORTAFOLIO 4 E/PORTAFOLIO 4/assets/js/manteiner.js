...
function chargeManteiner(listadoProductos){  
  ...
  listadoProductos.forEach(producto => {    
    const productoEnProductos = productos.find(p => p.sku === producto.sku);
    if(productoEnProductos){
    acumuladorFilas += `
      <tr>
        <th scope="row">${productoEnProductos.sku}</th>
        <td>${productoEnProductos.nombre}</td>
        <td>${productoEnProductos.descripcion}</td>
        <td>${productoEnProductos.precio}</td>
        <td>${productoEnProductos.descuento}</td>
        <td>${productoEnProductos.stock}</td>
      </tr>
    `;}
    else{
      acumuladorFilas += `
